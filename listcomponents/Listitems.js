...
export function ListItems({ item, listId }) {
  ...
  function checkIfDone(isDone) {
    if (isDone) {
      ...
      return isToday(item.expiryDate)
        ? 'Expires Today!'
        : 'Expires: ' + getDateToString(item.expiryDate);
    }
  }
      ...
      >
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View style={{ flexDirection: 'column' }}>
            ...
            <Text
              style={{ color: isOverdue(item.expiryDate) ? 'red' : '#777' }}
            >
              {checkIfDone(item.isFinished)}
            </Text>
          </View>
          <View
            style={{
          ...
          >
            ...
            />
            <View
              style={{ width: 10, height: '90%', backgroundColor: '#f2f2f2' }}
            />
          </View>
        </View>
      </Pressable>
